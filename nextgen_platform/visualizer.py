...
# ─── Chart 5: Candidate Priority Map (Figure 2 in plan.md) ────────────────────

def candidate_priority_map_chart(df: pd.DataFrame) -> str:
    """
    Figure 2 from plan.md: Candidate Priority Map (Bubble Quad-Plot).
    X-axis: -log10(%Rank_EL) (Presentation)
    Y-axis: IEDB Immunogenicity Score (T-Cell Response)
    Bubble Size: HLA Breadth / Population Coverage
    Color Code: Tier 1 (Teal), Tier 2 (Amber), Tier 3 (Gray)
    """
    if df.empty or "Rank_EL" not in df.columns:
        return "<p style='color:#8b949e; text-align:center; padding:40px;'>No data available for Candidate Priority Map.</p>"

    df = df.copy()
    valid_df = df[df["Rank_EL"] > 0].copy()
    valid_df["log_rank"] = -np.log10(valid_df["Rank_EL"] / 100.0)
    valid_df["imm_score"] = valid_df["Immunogenicity_Score"].fillna(0.0)

    if "Tier" not in valid_df.columns:
        valid_df["Tier"] = "Tier 3"

    tier_colors = {
        "Tier 1": COLORS["strong"],      # #00f5c4
        "Tier 2": COLORS["weak"],        # #f5a623
        "Tier 3": COLORS["nonbinder"],   # #6b7280
    }

    fig = go.Figure()
    for tier in ["Tier 1", "Tier 2", "Tier 3"]:
        sub = valid_df[valid_df["Tier"] == tier]
        if sub.empty:
            continue
        
        # Calculate bubble size based on HLA breadth score or default size
        sizes = (sub["HLA_Breadth_Score"].fillna(0.2) * 20 + 8).tolist() if "HLA_Breadth_Score" in sub.columns else [10] * len(sub)

        fig.add_trace(go.Scattergl(
            x=sub["log_rank"].tolist(),
            y=sub["imm_score"].tolist(),
            mode="markers",
            name=f"🌟 {tier}" if tier == "Tier 1" else f"⚡ {tier}" if tier == "Tier 2" else f"📁 {tier}",
            marker=dict(color=tier_colors[tier], size=sizes, opacity=0.85),
            customdata=sub[["Peptide", "MHC", "Rank_EL", "Composite_Score"]].values.tolist() if "Composite_Score" in sub.columns else sub[["Peptide", "MHC", "Rank_EL"]].values.tolist(),
            hovertemplate=(
                "<b>Peptide:</b> %{customdata[0]}<br>"
                "<b>HLA:</b> %{customdata[1]}<br>"
                "<b>%Rank EL:</b> %{customdata[2]:.3f}%<br>"
                "<b>Immunogenicity Score:</b> %{y:.4f}<br>"
                + ("<b>Composite Score:</b> %{customdata[3]:.4f}<extra></extra>" if "Composite_Score" in sub.columns else "<extra></extra>")
            )
        ))

    # Add quadrant threshold lines
    sb_y = -np.log10(0.5 / 100.0) # ~2.301
    fig.add_vline(x=sb_y, line_dash="dash", line_color=COLORS["strong"], annotation_text="Strong Presentation (%Rank < 0.5%)", annotation_position="top right")
    fig.add_hline(y=0.0, line_dash="dash", line_color=COLORS["primary"], annotation_text="Positive Immunogenicity (> 0)", annotation_position="bottom right")

    fig.update_layout(
        paper_bgcolor=COLORS["bg"],
        plot_bgcolor=COLORS["panel"],
        font=dict(color=COLORS["text"], family="Inter, sans-serif"),
        title=dict(text="Figure 2: Candidate Priority Map (−log₁₀ Rank vs Immunogenicity)", font=dict(size=15, color=COLORS["text"])),
        xaxis=dict(title="Presentation Score: −log₁₀(%Rank EL / 100)", gridcolor=COLORS["border"], zerolinecolor=COLORS["border"]),
...
# ─── Chart 6: Multimodal Tier Distribution Bar Chart ──────────────────────────

def tier_distribution_chart(df: pd.DataFrame) -> str:
    """
    Bar Chart showing count of candidates classified into Tier 1, Tier 2, Tier 3.
    """
    if df.empty or "Tier" not in df.columns:
        return "<p style='color:#8b949e; text-align:center; padding:40px;'>No Tier data available.</p>"

    counts = df["Tier"].value_counts().reindex(["Tier 1", "Tier 2", "Tier 3"], fill_value=0)
    categories = ["Tier 1 (High Priority)", "Tier 2 (Medium Priority)", "Tier 3 (Backup)"]
    values = [counts["Tier 1"], counts["Tier 2"], counts["Tier 3"]]

    fig = go.Figure(data=[go.Bar(
        x=categories,
        y=values,
        marker_color=[COLORS["strong"], COLORS["weak"], COLORS["nonbinder"]],
        text=values,
        textposition="auto",
    )])
    ...
    fig.update_layout(
        ...
        plot_bgcolor=COLORS["panel"],
        font=dict(color=COLORS["text"], family="Inter, sans-serif"),
        title=dict(text="Multimodal Tier Candidate Distribution", font=dict(size=15, color=COLORS["text"])),
        xaxis=dict(title="Candidate Priority Tier", gridcolor=COLORS["border"]),
        yaxis=dict(title="Candidate Count", gridcolor=COLORS["border"]),
        height=380,
...
# ─── Chart 7: Dual-Layer Immunogenicity Concordance (IEDB vs ImmunoStruct) ─────

def dual_immunogenicity_concordance_chart(df: pd.DataFrame) -> str:
    """
    Dual-Layer Scatter Plot comparing IEDB sequence-based score vs ImmunoStruct 3D multimodal score.
    Highlights Structure-Driven Epitopes discovered exclusively by ImmunoStruct 3D features.
    """
    if df.empty or "ImmunoStruct_Score" not in df.columns:
        return "<p style='color:#8b949e; text-align:center; padding:40px;'>ImmunoStruct scores not available for this run.</p>"

    valid_df = df.copy()
    valid_df["iedb_x"] = valid_df["Immunogenicity_Score"].fillna(-0.25)
    valid_df["struct_y"] = valid_df["ImmunoStruct_Score"].fillna(0.50)

    if "Concordance" not in valid_df.columns:
        from immunostruct_api import evaluate_concordance
        valid_df["Concordance"] = valid_df.apply(lambda r: evaluate_concordance(r["Immunogenicity_Score"], r["ImmunoStruct_Score"]), axis=1)

    category_colors = {
        "🌟 Double Validated": COLORS["strong"],      # #00f5c4
        "💡 Structure-Driven": "#a371f7",            # Purple Accent
        "⚠️ Sequence-Only":    COLORS["weak"],        # #f5a623
        "📁 Low Priority":     COLORS["nonbinder"],   # #6b7280
    }

    fig = go.Figure()
    for cat in ["🌟 Double Validated", "💡 Structure-Driven", "⚠️ Sequence-Only", "📁 Low Priority"]:
        sub = valid_df[valid_df["Concordance"] == cat]
        if sub.empty:
            continue

        fig.add_trace(go.Scattergl(
            x=sub["iedb_x"].tolist(),
            y=sub["struct_y"].tolist(),
            mode="markers",
            name=cat,
            marker=dict(color=category_colors.get(cat, COLORS["primary"]), size=10, opacity=0.85),
            customdata=sub[["Peptide", "MHC", "Rank_EL", "Composite_Score"]].values.tolist() if "Composite_Score" in sub.columns else sub[["Peptide", "MHC", "Rank_EL"]].values.tolist(),
            hovertemplate=(
                "<b>Peptide:</b> %{customdata[0]}<br>"
                "<b>HLA:</b> %{customdata[1]}<br>"
                "<b>IEDB Score:</b> %{x:.4f}<br>"
                "<b>ImmunoStruct 3D Score:</b> %{y:.4f}<br>"
                "<b>Concordance:</b> " + cat + "<extra></extra>"
            )
        ))

    # Add quadrant threshold lines
    fig.add_vline(x=0.10, line_dash="dash", line_color=COLORS["primary"], annotation_text="IEDB Sequence Threshold (>0.10)", annotation_position="top right")
    fig.add_hline(y=0.50, line_dash="dash", line_color="#a371f7", annotation_text="ImmunoStruct 3D Threshold (≥0.50)", annotation_position="bottom right")

    fig.update_layout(
        paper_bgcolor=COLORS["bg"],
        plot_bgcolor=COLORS["panel"],
        font=dict(color=COLORS["text"], family="Inter, sans-serif"),
        title=dict(text="Dual Immunogenicity Concordance: IEDB (Seq) vs ImmunoStruct (3D Graph)", font=dict(size=15, color=COLORS["text"])),
        xaxis=dict(title="IEDB T-Cell Immunogenicity Score (Sequence-based)", gridcolor=COLORS["border"], zerolinecolor=COLORS["border"]),
        yaxis=dict(title="ImmunoStruct 3D Graph Immunogenicity Score", gridcolor=COLORS["border"], zerolinecolor=COLORS["border"]),
        height=380,
        margin=dict(t=50, b=50, l=70, r=30),
        legend=dict(bgcolor=COLORS["panel"], bordercolor=COLORS["border"], borderwidth=1),
        autosize=True,
...
def build_dashboard_html(df: pd.DataFrame, job_meta: dict) -> Dict[str, str]:
    ...
    return {
        ...
        "imm_quad":          candidate_priority_map_chart(df),
        "pop_coverage":      tier_distribution_chart(df),
        "concordance_chart": dual_immunogenicity_concordance_chart(df),
    }
